...
def two_int_sequence_decoder(encoded: bytes) -> Tuple[int, int]:
    ...
    if encoded[0] != 0x30:
        raise ValueError(
            "Encoded sequence must start with 0x30 byte, but got %s" % encoded[0]
        )
    ...
    if len(empty) != 0:
        raise ValueError(
            "Encoded sequence must not contain any trailing data, but had %r" % empty
        )
...
def _decode_int(encoded: bytes) -> Tuple[int, bytes]:
    ...
    if encoded[0] != 0x02:
        raise ValueError(
            "Encoded value must be an integer, starting with on 0x02 byte, but got %s"
            % encoded[0]
        )
