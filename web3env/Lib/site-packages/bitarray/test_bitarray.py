...
class CreateObjectTests(unittest.TestCase, Util):
    ...
    def test_string01(self):
        for s in ("0010111", "0010111", "0010 111", "0010 111", "0010_111", "0010_111"):
            a = bitarray(s)
    ...
    def test_rawbytes_invalid(self):
        ...
        for blob in b"\x00", b"\x07\x80":
            self.assertRaisesMessage(
                ValueError, "endianness missing for pickle", bitarray, blob
            )
    ...
    def test_setslice_random(self):
        for a in self.randombitarrays(start=1):
            ...
            for _ in range(10):
                step = self.rndsliceidx(la) or None
                s = slice(self.rndsliceidx(la), self.rndsliceidx(la), step)
                lb = randint(0, 10) if step is None else self.calc_slicelength(s, la)
                b = bitarray(lb)
                c = bitarray(a)
                c[s] = b
                self.check_obj(c)
