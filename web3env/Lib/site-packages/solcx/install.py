...
def _get_os_name() -> str:
    ...
    if sys.platform == "win32":
        return "windows"
    raise OSError(
        f"Unsupported OS: '{sys.platform}' - py-solc-x supports Linux, OSX and Windows"
    )
...
def import_installed_solc(solcx_binary_path: Union[Path, str] = None) -> List[Version]:
    ...
    for path in path_list:
        ...
        copy_path = get_solcx_install_folder(solcx_binary_path).joinpath(
            f"solc-v{version}"
        )
        if _get_os_name() == "windows":
            copy_path.mkdir()
...
def _select_pragma_version(
    pragma_string: str, version_list: List[Version]
) -> Optional[Version]:
...
def install_solc_pragma(
    ...
    if install:
        install_solc(
            version, show_progress=show_progress, solcx_binary_path=solcx_binary_path
        )
...
def get_compilable_solc_versions(headers: Optional[Dict] = None) -> List[Version]:
    ...
    for release in data.json():
        ...
        asset = next(
            (i for i in release["assets"] if re.match(pattern, i["name"])), False
        )
        if asset:
...
def get_installed_solc_versions(
    ...
    return sorted(
        [Version(i.name[6:]) for i in install_path.glob("solc-v*")], reverse=True
    )
    ...
    with process_lock:
        ...
        try:
            ...
            raise SolcInstallationError(
                f"Solc binary for v{version} is not available for this OS"
            )
    ...
    with process_lock:
        ...
        try:
            ...
            subprocess.check_call(
                ["sh", temp_path.joinpath("scripts/install_deps.sh")],
...
def _validate_installation(
    ...
    if installed_version != version:
        warnings.warn(
            f"Installed solc version is v{installed_version}", UnexpectedVersionWarning
        )
    if not _default_solc_binary:
